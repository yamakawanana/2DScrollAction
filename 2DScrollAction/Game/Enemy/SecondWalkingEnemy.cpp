...
namespace {
  ...
  /// <summary>
  /// X座標の開始位置を定数kStartSecondWalkingEnemyXに格納する
  /// </summary>
  static const int kStartSecondWalkingEnemyX = 700;

  /// <summary>
  /// Y座標の開始位置を定数kStartSecondWalkingEnemyYに格納する
  /// </summary>
  static const int kStartSecondWalkingEnemyY = 496;
  ...
  /// </summary>
  static const float kChangeTime[] = { 0.05f, 0.1f, 0.15f, 0.2f ,0.25f ,0.3f ,0.35f ,0.4f ,0.45f ,0.5f ,0.55f, 0.6f ,0.65f ,0.7f ,0.75f };

  /// <summary>
  /// 画像の横向きの分割数を配列kPartitionNumbersに格納する
  /// </summary>
  static const int kPartitionNumbers[] = { kSecondWalkingEnemyIdlingX, kSecondWalkingEnemyWalkingX, kSecondWalkingEnemyHitX, 1, 1 };
...
/// <param>なし</param>
/// <returns>なし</returns>
SecondWalkingEnemy::SecondWalkingEnemy(Task task_id, TaskManager * task_manager)
  : Task(task_id)
  ...
  , second_enemy_drop_accumulation_time_(0.0f)
  , fall_accumulation_count_(0.0f)
  , x_coordinate_(kStartSecondWalkingEnemyX)
  , y_coordinate_(kStartSecondWalkingEnemyY)
  , width_(0)
  , height_(0)
  , half_width_(0.0f)
  , half_height_(0.0f)
  ...
  , jump_speed_(0.0f)
{
  DEBUG_PRINT(SecondWalkingEnemyのコンストラクタ);
}
...
/// <param>なし</param>
/// <returns>なし</returns>
SecondWalkingEnemy::~SecondWalkingEnemy() {
  DEBUG_PRINT(SecondWalkingEnemyのデストラクタ);
  //wait_accumulation_time_ = 0.0f;
...
/// <returns>なし</returns>
void SecondWalkingEnemy::UpdateFrame(float time) {
  if (now_phase_ != kStop && now_phase_ != kFinish && now_phase_ != kPreraration && is_move_) {
    ...
    //足元が地面と接しているか判定する
    is_ground_ = collision_->IsFootGround(x_coordinate_, y_coordinate_, kSnailSizeY);

    //右に歩く状態の場合
    if (status_type_ == kWalkRight) {
      x_coordinate_ = x_coordinate_ + kTwo;
      bool is_right = collision_->IsHitRight(x_coordinate_, y_coordinate_, kSnailSizeX);
      if (is_right) {
    ...
    //左に歩く状態の場合
      ...
      x_coordinate_ = x_coordinate_ - kTwo;
      bool is_left = collision_->IsHitLeft(x_coordinate_, y_coordinate_, kSnailSizeX);
      if (is_left) {
    ...
    //倒された場合
      ...
      if (second_enemy_drop_accumulation_time_ == kJumpLimitTime) {
...
/// <param>なし</param>
/// <returns>なし</returns>
void SecondWalkingEnemy::RenderFrame() {
  //DEBUG_PRINT(SecondWalkingEnemyの毎フレーム描画処理);
  if (now_phase_ != kFinish && now_phase_ != kPreraration) {
    ...
    //グラフィックハンドルを配列に格納する
    int graphic_handles[] = { idling_gr_handle_[(index_) % kSecondWalkingEnemyIdlingX], walking_gr_handle_[(index_) % kSecondWalkingEnemyWalkingX], hit_gr_handle_[(index_) % kSecondWalkingEnemyHitX], jump_gr_handle_, fall_gr_handle_ };
    ...
    if (status_type_ == kDrop) {
      DrawGraph((x_coordinate - difference_x_pos_), (y_coordinate), hit_gr_handle_[kSecondWalkingEnemyHitX - 1], TRUE);
    }
  }
}
...
/// <returns>なし</returns>
void SecondWalkingEnemy::SetIdlingGrHandle(std::array<int, kSecondWalkingEnemyIdlingX> idling_gr_handle) {
...
int SecondWalkingEnemy::GetIdlingGrHandle(int element_num) {
...
/// <returns>なし</returns>
void SecondWalkingEnemy::SetWalkingGrHandle(std::array<int, kSecondWalkingEnemyWalkingX> walking_gr_handle) {
...
int SecondWalkingEnemy::GetWalkingGrHandle(int element_num) {
...
/// <returns>なし</returns>
void SecondWalkingEnemy::SetHitGrHandle(std::array<int, kSecondWalkingEnemyHitX> hit_gr_handle) {
...
int SecondWalkingEnemy::GetHitGrHandle(int element_num) {
...
/// <returns>なし</returns>
void SecondWalkingEnemy::SetXPosition(int x_coordinate) {
...
/// <param>なし</param>
/// <returns>X座標</returns>
int SecondWalkingEnemy::GetXPosition() {
...
/// <returns>なし</returns>
void SecondWalkingEnemy::SetYPosition(int y_coordinate) {
...
/// <param>なし</param>
/// <returns>Y座標</returns>
int SecondWalkingEnemy::GetYPosition() {
...
/// <param>画像の幅</param>
/// <returns>なし</returns>
void SecondWalkingEnemy::SetWidth(int size_x) {
  width_ = size_x;
}
...
int SecondWalkingEnemy::GetWidth() {
  return width_;
}
...
/// <param>画像の幅</param>
/// <returns>なし</returns>
void SecondWalkingEnemy::SetHeight(int size_y) {
  height_ = size_y;
}
...
int SecondWalkingEnemy::GetHeight() {
  return height_;
}
...
/// <param>画像の幅の半分</param>
/// <returns>なし</returns>
void SecondWalkingEnemy::SetHalfWidth(float size_half_x) {
...
float SecondWalkingEnemy::GetHalfWidth() {
  return half_width_;
}
...
/// <returns>なし</returns>
void SecondWalkingEnemy::SetHalfHeight(float size_half_y) {
...
float SecondWalkingEnemy::GetHalfHeight() {
  return half_height_;
}
...
/// <returns>なし</returns>
void SecondWalkingEnemy::SetDirectionType(DirectionType direction) {
...
SecondWalkingEnemy::DirectionType SecondWalkingEnemy::GetDirectionType() {
...
/// <returns>なし</returns>
void SecondWalkingEnemy::SetPhaseType(PhaseType type) {
  now_phase_ = type;
}
...
/// <param>なし</param>
/// <returns>方向の種類</returns>
SecondWalkingEnemy::PhaseType SecondWalkingEnemy::GetPhaseType() {
  return now_phase_;
}
...
/// <returns>なし</returns>
void SecondWalkingEnemy::SetDisplayXPosition(float display_x_coordinate) {
...
float SecondWalkingEnemy::GetDisplayXPosition() {
...
/// <returns>なし</returns>
void SecondWalkingEnemy::SetDisplayYPosition(float display_y_coordinate) {
...
float SecondWalkingEnemy::GetDisplayYPosition() {
...
int SecondWalkingEnemy::GetPieceTop() {
  //int x, y;
...
int SecondWalkingEnemy::GetPieceBottom() {
  //int x, y;
...
int SecondWalkingEnemy::GetPieceLeft() {
  //int x, y;
...
int SecondWalkingEnemy::GetPieceRight() {
  //int x, y;
...
/// <param = ""></param>
/// <returns>なし</returns>
void SecondWalkingEnemy::SetHitBottom(bool is_hit_bottom) {
...
/// <param = ""></param>
/// <returns>なし</returns>
void SecondWalkingEnemy::SetHitRight() {
  is_hit_right_ = true;
}
...
/// <param = ""></param>
/// <returns>なし</returns>
void SecondWalkingEnemy::SetHitLeft() {
  is_hit_left_ = true;
}
...
/// <param = ""></param>
/// <returns>なし</returns>
void SecondWalkingEnemy::SetHit(bool is_hit) {
  is_hit_ = is_hit;
}
...
/// <param name>なし</param>
/// <returns>なし</returns>
int SecondWalkingEnemy::CheckIsHitEnemyBasesBottom() {
...
/// <returns>なし</returns>
void SecondWalkingEnemy::SetMoveSoundHandle(int move_sound_handle) {
...
int SecondWalkingEnemy::GetMoveSoundHandle() {
...
/// <returns>なし</returns>
void SecondWalkingEnemy::SetXMoveSoundHandle(int move_handle) {
...
int SecondWalkingEnemy::GetXMoveSoundHandle() {
  return move_handle_;
}
...
/// <returns>なし</returns>
void SecondWalkingEnemy::SetSoundVolume(int sound_volume) {
...
/// <returns>なし</returns>
void SecondWalkingEnemy::SetJumpGrHandle(int jump_gr_handle) {
...
int SecondWalkingEnemy::GetJumpGrHandle() {
...
/// <returns>なし</returns>
void SecondWalkingEnemy::SetFallGrHandle(int fall_gr_handle) {
...
int SecondWalkingEnemy::GetFallGrHandle() {
...
/// <returns>なし</returns>
void SecondWalkingEnemy::SetDifferenceXpos(int difference_x_pos) {
...
/// <returns>なし</returns>
void SecondWalkingEnemy::SetStatusType(StatusType status_type) {
...
SecondWalkingEnemy::StatusType SecondWalkingEnemy::GetStatusType() {
  return status_type_;
}
...
/// <returns>なし</returns>
float SecondWalkingEnemy::CalculateJumpSpeed(float time) {

  second_enemy_drop_accumulation_time_ = second_enemy_drop_accumulation_time_ + time;
  //累積時間が一定時間を超えない様にする
  if (second_enemy_drop_accumulation_time_ > kJumpLimitTime) {
    second_enemy_drop_accumulation_time_ = kJumpLimitTime;
  }

  //累積時間によって飛び上がる高さを計算する
  float jump_speed = kInitialRate * (kFloatOne - second_enemy_drop_accumulation_time_ / kJumpLimitTime);
...
/// <param ="time">毎フレーム更新時間</param>
/// <returns>なし</returns>
void SecondWalkingEnemy::FallEnemy(float time, float limit_time) {
...
/// <param = ""></param>
/// <returns>なし</returns>
void SecondWalkingEnemy::SetMove(bool is_move) {
  is_move_ = is_move;
}
...
/// <returns>なし</returns>
void SecondWalkingEnemy::SetStartMoveX(int start_move_x) {
...
int SecondWalkingEnemy::GetStartMoveX() {
